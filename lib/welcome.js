      ...
      if (anu.action == 'add' && (iswel || setting.auto_welcomeMsg)) {
        ...
        if (isSetWelcome(anu.id, set_welcome_db)) {
          ...
          await NanoBotz.sendMessage(anu.id, {
            text: `Hiii @${num.split('@')[0]}\nWelcome to ${groupName},Dont forget to introduce yourself`,
          }, {});
        }
        ...
        if (isSetLeft(anu.id, set_left_db)) {
          ...
          await NanoBotz.sendMessage(anu.id, {
            text: `User *@${num.split('@')[0]}* left/removed from the group *${groupName}*`,
          }, {});
        }
      } else if (anu.action == 'promote') {
         NanoBotz.sendMessage(anu.id, {
            text: `User @${num.split('@')[0]}\nYou have been promoted in ${groupName}\n`,
          }, {});
      } else if (anu.action == 'demote') {
         NanoBotz.sendMessage(anu.id, {
            text: `User @${num.split('@')[0]}\nYou have been demoted in ${groupName}\n`,
          }, {});
      }
    }
  } catch (err) {
